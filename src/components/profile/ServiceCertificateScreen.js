...
const ServiceCertificateScreen = ({ onBack }) => {
    ...
    try {
      ...
      const resp = await fetch(API_ENDPOINTS.SERVICE_CERT_SUBMIT, {
        ...
        body: JSON.stringify({
          ...
          has_mouse: assetForm.mouse ? 'Yes' : 'No',
          has_keyboard: assetForm.keyboard ? 'Yes' : 'No',
          has_laptop_stand: assetForm.stand ? 'Yes' : 'No'
        })
      });
  ...
  useEffect(() => {
    const fetchCurrentAssets = async () => {
      const uid = employeeId || user?.employee_id || user?.id; // Priority to ID from URL
      if (!uid) return;
      ...
      try {
        ...
        if (res.ok) {
          ...
          console.log("[Assets Feed] Syncing for ID:", uid, data);
          const asset = Array.isArray(data) ? (data.length > 0 ? data[0] : null) : data;

          if (asset) {
            const isYes = (val) => String(val || '').toLowerCase().trim() === 'yes';

            setAssetForm({
              brand: asset.laptop_details || asset.asset_name || '',
              serial: asset.serial_no || '',
              mouse: isYes(asset.mouse),
              keyboard: isYes(asset.keyboard),
              stand: isYes(asset.laptop_stand),
              mobile: isYes(asset.mobile),
              camera: isYes(asset.camera),
              headphone: isYes(asset.earphone_headphone),
              tablet: isYes(asset.tablet),
              pendrive: isYes(asset.pendrive),
              rufpad: isYes(asset.ruf_pad)
            });
            setAssetStatus(asset.status || 'Submitted');
    ...
    fetchCurrentAssets();
  }, [employeeId, user]); // Refetch if user or viewed profile changes
  ...
  const handleAssetSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const resp = await fetch(API_ENDPOINTS.ASSETS, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
        ...
        body: JSON.stringify({
          ...
          laptop_details: assetForm.brand,
          mouse: assetForm.mouse ? 'Yes' : 'No',
          keyboard: assetForm.keyboard ? 'Yes' : 'No',
          laptop_stand: assetForm.stand ? 'Yes' : 'No',
          mobile: assetForm.mobile ? 'Yes' : 'No',
          camera: assetForm.camera ? 'Yes' : 'No',
          earphone_headphone: assetForm.headphone ? 'Yes' : 'No',
          tablet: assetForm.tablet ? 'Yes' : 'No',
          pendrive: assetForm.pendrive ? 'Yes' : 'No',
          ruf_pad: assetForm.rufpad ? 'Yes' : 'No',
          status: 'Pending Audit' // Initial status for record
        })
      });
            ...
            <div style={{ ...s.card, order: hasSubmittedAssets ? 2 : 1 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                ...
                {hasSubmittedAssets && (
                  <div style={{
                    padding: '6px 12px',
                    backgroundColor: assetStatus === 'Approved' ? '#f0fdf4' : '#fff7ed',
                    border: `1px solid ${assetStatus === 'Approved' ? '#bbf7d0' : '#ffedd5'}`,
                    borderRadius: '20px',
                    display: 'flex',
                    alignItems: 'center',
                    gap: '6px'
                  }}>
                    <div style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: assetStatus === 'Approved' ? '#10b981' : '#f59e0b' }}></div>
                    <span style={{ fontSize: '10px', fontWeight: '800', color: assetStatus === 'Approved' ? '#166534' : '#9a3412', textTransform: 'uppercase' }}>
                      {assetStatus || 'Pending Audit'}
                    </span>
                  </div>
                )}
              </div>
              ...
              <form onSubmit={handleAssetSubmit}>
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '20px', marginBottom: '20px' }}>
                  <div>
                    <label style={s.label}>Laptop Brand / Model / Serial No</label>
                    <input
                      type="text"
                      style={s.select}
                      placeholder="e.g. Lenovo Ryzen 7, Serial No: PF5P6L2E"
                ...
                <div style={{ backgroundColor: '#f8fafc', padding: '20px', borderRadius: '24px', marginBottom: '25px', border: '1.5px solid #f1f5f9' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(5, 1fr)', gap: '10px' }}>
                    {[
                      ...
                      { label: 'Laptop Stand', key: 'stand', icon: <Monitor size={16} /> },
                      { label: 'Company Mobile', key: 'mobile', icon: <Smartphone size={16} /> },
                      { label: 'Earphones / Headphones', key: 'headphone', icon: <Headphones size={16} /> },
                      { label: 'External Camera', key: 'camera', icon: <Camera size={16} /> },
                      { label: 'Tablet', key: 'tablet', icon: <Tablet size={16} /> },
                      { label: 'Pendrive / Storage', key: 'pendrive', icon: <Database size={16} /> },
                      { label: 'Ruf Pad / Notebook', key: 'rufpad', icon: <FileText size={16} /> }
                    ].map(item => {
