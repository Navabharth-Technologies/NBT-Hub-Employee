...
const InternFunQuizScreen = ({ onBack }) => {
  ...
  const myLeaderboardData = leaderboard.find(s => {
    ...
    return possibleUserKeys.some(key => {
      ...
      return cleanSId === cleanKey || cleanSName === cleanKey || cleanSId === `${prefix}${cleanKey}`;
    });
  });
  ...
  const fetchScores = async () => {
    try {
      ...
      let uid = String(rawUid || '').split(':')[0].trim();
      
      const isIntern = String(user?.role || '').toUpperCase().includes('INTERN') || String(user?.designation || '').toUpperCase().includes('INTERN');
      if (/^\d+$/.test(uid)) {
        uid = isIntern ? `INT${uid}` : `EMP${uid}`;
      }
  ...
  const handleSendTotalResults = async () => {
    ...
    try {
      ...
      const rawUid = user?.employee_id || user?.userId || user?.id || user?.employeeId || user?.uid;
      let uid = String(rawUid).includes(',')
          ...
          return displayList.map((p, i) => {
            const isMe = (() => {
              ...
              return possibleUserKeys.some(key => {
                const cleanKey = String(key || '').split(':')[0].trim().toLowerCase();
                if (!cleanKey) return false;
                return cleanPId === cleanKey || cleanPName === cleanKey || cleanPId === `${prefix}${cleanKey}`;
              });
            })();
            return (
              <div key={i} style={{
