...
const AwardsScreen = ({ onBack }) => {
    ...
    const allRewards = globalRewardsFeed.filter(r => {
        ...
        const isIntern = String(user?.role || '').toUpperCase().includes('INTERN') || String(user?.designation || '').toUpperCase().includes('INTERN');
        const prefix = isIntern ? 'int' : 'emp';

        const checkMatch = (myVal) => {
            if (!myVal) return false;
            return rEmployeeId === myVal || rUserId === myVal || rEmpId === myVal || 
                   rEmployeeId === `${prefix}${myVal}` || rUserId === `${prefix}${myVal}` || rEmpId === `${prefix}${myVal}`;
        };

        const isMyReward = checkMatch(myId) || checkMatch(myUid) || checkMatch(myUserId) || (myEmail && checkMatch(myEmail));
